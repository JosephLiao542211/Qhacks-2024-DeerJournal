...
async function getMood(prev_conversation_history: any) {
  var req =
    prev_conversation_history[prev_conversation_history.length - 1].content;
  var res = "";
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a highly attuned and emotionally receptive empath to children. You categorize all the responses that are inputted to you as either 'happy' or 'sad'. All the responses inputted towards you are children in one of the two emotionally states mentioned above recounting their day. You respond with either 'happy' or 'sad' depending on what mood you believe the child is feeling about their day. If the string 'DO_NOT_REPLY' is found at the end of any prompts or chat history, reply only with the string 'no_response'.",
      },
      {
        role: "user",
        content: userR,
...
async function secondQ(model: string, prev_conversation_history: any) {
  ...
  new_conversation_history.push({
    role: "user",
    content:
      "Now that I have told you about my day, specifically my last response, \
